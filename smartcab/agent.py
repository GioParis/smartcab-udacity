...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.8, decay_rate=0.9975):
    ...
    def choose_action(self, state):
        ...
        if self.learning == True or self.epsilon > random.random():
         maxQ = self.get_maxQ(state)
         maxQs = []
         for act in self.Q[state]:
             if self.Q[state][act] == maxQ:
                 maxQs.append(act)
        action = random.choice(maxQs)
...
def run():
    ...
    sim.run(n_test = 500, tolerance=0.015)
